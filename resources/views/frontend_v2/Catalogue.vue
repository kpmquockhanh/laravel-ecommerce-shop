        ...
        'shop-catalogue',
         `${layout}-view`
            ...
            <div class="row items-grid">
              <div class="product" :class="[`product-${layout}`, {'col-md-4 col-xs-6': layout === 'grid'}]"
                   v-for="product in products">
                <ProductItem :type="layout" :product="product"/>
              </div>
            </div>
          </div>
          <PaginationV2 :total="total" :per-page="perPage" v-model:current-page="currentPage"/>
        </div>
...
export default {
  ...
  props: {},
  setup() {
    const {products, productMeta, fetchProducts, isLoadingProducts, currentPage, perPage, total, onChangeSort, querySort} = useProduct()
